function fwtm_val = fwtm_kwave(f, x, plot_fwtm)
...
while f(index) > 0.1*f_max && index > 1
    index = index - 1;
%     if index < 1
...
c = f(index) - x(index)*m;
i_leading = (0.1*f_max  - c)/m;

% setup the indexing variables for finding the trailing edge
index = i_max;

% loop until the index at half maximum is found
while f(index) > 0.1*f_max && index < length(f)
    index = index + 1;
%     if index > length(f)
...
if(sum(f(:)~=0))
    m = (f(index-1) - f(index))/(x(index-1) - x(index));
    c = f(index) - x(index)*m;
    i_trailing = (0.1*f_max  - c)/m;
    fwtm_val = abs(i_trailing - i_leading);
end  

% plot the function and the FWTM if required
if plot_fwtm 
    plot(x, f, 'b-');
    xlabel('x');
    ylabel('f(x)');    
    hold on;
    plot([i_leading i_trailing], [0.1*f_max 0.1*f_max], 'r*-');
    title(['FWTM: ' num2str(fwtm_val) ]);
end
