function [] = plot_lc_new(rho,eta,marker,ps,reg_param,metodo)
...
switch(metodo)
    case 'zibetti'
        plot(rho(2:end-1),eta(2:end-1)), ax = axis;
        ax=[0 max(ax(:))/1.5 0 max(ax(:))/1.5];
        ...
        xlabel('residual norm || g - H f ||_2/sqrt(Tr(H^TH))')
        if (ps==1)
          ylabel('solution norm || f ||_2/sqrt(M)')
        else
          ylabel('solution semi-norm || L f ||_2')
        end
        title('Sqrt Linear L-curve')
    case 'hansen'
%         loglog(rho(2:end-1),eta(2:end-1),[marker 'k'],'LineWidth',1), ax = axis;       
        if (max(eta)/min(eta) > 10 | max(rho)/min(rho) > 10)
            if (nargin < 5)
                loglog(rho,eta,[marker 'k'],'LineWidth',3), ax = axis; 
            else
                loglog(rho,eta,[marker 'k'],'LineWidth',3), ax = axis; 
                hold on,
                loglog(rho(ni:ni:n),eta(ni:ni:n),'x','LineWidth',3), axis(ax)
                HoldState = ishold; hold on;
                for k = ni:ni:n
                    text(rho(k),eta(k),num2str(reg_param(k)));
                end
